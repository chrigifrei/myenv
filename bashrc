...
if [ -e ~/myenv ]; then
    docroot='~/myenv/knowhow'
elif [ -e /opt/myenv ]; then
    docroot='/opt/myenv/knowhow'
elif [ -e ~/projects/myenv ]; then
    docroot='~/projects/myenv/knowhow'
else
    docroot='~'
fi
...
# OSX aliases
    ...
    alias dns-sniff='sudo tcpdump -A -s0 -nni en0 port 53'
fi
